...
{
    // const float k = 1.f / vec3_len(v);
    vec3_scale(r, v, 1.0f / vec3_len(v)/*k*/);
}
...
static inline void vec4_norm(vec4 r, vec4 v)
{
    ...
    vec4_scale(r, v, /*k*/1.f / vec4_len(v));
}
...
static inline void mat4_translate_in_place(mat4 M, float x, float y, float z)
{
    ...
    // }
    mat4_row(r, M, 0);
    M[3][0] += vec4_mul_inner(r, t);
    mat4_row(r, M, 1);
    M[3][1] += vec4_mul_inner(r, t);
    mat4_row(r, M, 2);
    M[3][2] += vec4_mul_inner(r, t);
    mat4_row(r, M, 3);
    M[3][3] += vec4_mul_inner(r, t);
}
